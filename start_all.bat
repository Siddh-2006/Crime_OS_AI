...
start "Legal Agent (8004)" cmd /k "call .venv\Scripts\activate && cd services\legal_agent && uvicorn app.main:app --port 8004 --reload"
...
start "Complaint Intelligence (8001)" cmd /k "call .venv\Scripts\activate && cd services\complaint_intelligence && uvicorn app.main:app --port 8001 --reload"
...
@REM echo 6. Starting Indic Translation Model (Sarvam on Port 8004)...
@REM start "Sarvam Translation (8004)" cmd /k "echo Starting llama-server for Sarvam-1... && llama-server -m sarvam-1.Q8_0.gguf -c 2048 --port 8004"
